...
export default function StaffPowerBankPage() {
  ...
  const pbCode = params?.pbCode as string;
  ...
  const fetchData = async () => {
    setLoading(true);
    // Fetch Power Bank details and its location name
    ...
    if (pb) {
      setPbData(pb);
      // If rented, fetch the active rental details
      if (pb.status === 'rented') {
  ...
  const handleRentOut = async () => {
    ...
    // Update rental to active and power bank to rented
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'lost': return '#f97316'; // orange
