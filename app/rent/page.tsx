...
function RentContent() {
  ...
  useEffect(() => {
    ...
    // 2. Fetch Location and Owner Subaccount for 50/50 Split
    if (locationCode) {
      supabase
        .from("locations")
        .select("id, owner_id")
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    // FIX: Ensure email is always valid for Paystack
        ...
        supabase.from("rentals")
          ...
          .then(({ data, error }) => {
            if (data && !error) {
              ...
              alert("We could not find a successful payment. If money was deducted, please contact support.");
            }
          });
      }
    }
    ...
    try {
      ...
      const handler = paystackPop.setup({
        ...
        email: customerEmail, // Use the fixed email variable
        ...
        // Automatic 50/50 Split via Paystack Subaccount
